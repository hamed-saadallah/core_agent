...
export const AgentManagement: React.FC = () => {
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      {loading && <p className="text-gray-600 text-sm sm:text-base">Loading agents...</p>}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {localAgents.map((agent) => (
          <button
            ...
            className="bg-white rounded-lg shadow p-4 sm:p-6 hover:shadow-lg transition text-left"
          >
            <h3 className="text-base sm:text-lg font-semibold mb-2">{agent.name}</h3>
            <p className="text-gray-600 text-xs sm:text-sm mb-4">{agent.description}</p>
            {agent.model && (
              <div className="mb-4 p-2 sm:p-3 bg-purple-50 rounded border border-purple-200">
                <p className="text-xs font-medium text-purple-900 mb-1">Model:</p>
                <p className="text-xs sm:text-sm text-purple-800 font-semibold">{agent.model.name} (v{agent.model.version})</p>
                <p className="text-xs text-purple-700">Temperature: {agent.temperature?.toFixed(2) ?? agent.model?.temperature?.toFixed(2)}</p>
              </div>
            )}
            {agent.promptTemplate && (
              <div className="mb-4 p-2 sm:p-3 bg-blue-50 rounded border border-blue-200">
            ...
            <div className="flex items-center justify-between">
              <span className={`px-3 py-1 rounded-full text-xs sm:text-sm ${agent.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                {agent.status}
              </span>
            </div>
          </button>
      ...
      {localAgents.length === 0 && !loading && (
        <div className="text-center py-12">
          <p className="text-gray-500 text-sm sm:text-base">No agents found. Create one to get started.</p>
        </div>
      )}
