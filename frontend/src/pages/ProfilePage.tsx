...
export const ProfilePage: React.FC<ProfilePageProps> = ({ isEmbedded = false }) => {
  ...
  const content = (
    <div className="w-full">
      <div className={isEmbedded ? 'bg-white rounded-lg p-6' : 'bg-white rounded-lg shadow-xl p-8'}>
        <h1 className={`font-bold text-gray-900 mb-2 ${isEmbedded ? 'text-2xl' : 'text-3xl text-center'}`}>
          {isEmbedded ? 'My Profile' : 'Complete Your Profile'}
        </h1>
        {!isEmbedded && (
        ...
        {success && (
          <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg mb-6 text-sm">
            {isEmbedded ? 'Profile updated successfully!' : 'Profile updated successfully! Redirecting...'}
          </div>
        )}
